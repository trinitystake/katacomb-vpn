...
> **Status:** release (1.9.3). Connecting spends real funds. See
...
```bash
sudo apt install ./dist/katacomb-vpn_1.9.3_amd64.deb
```
...
```bash
chmod +x dist/katacomb-vpn-1.9.3.AppImage
./dist/katacomb-vpn-1.9.3.AppImage
```
...
```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./katacomb-vpn-1.9.3.AppImage
```
