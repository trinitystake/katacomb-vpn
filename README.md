...
> **Status:** release (1.9.1). Connecting spends real funds. See
...
```bash
sudo apt install ./dist/katacomb-vpn_1.9.1_amd64.deb
```
...
```bash
chmod +x dist/katacomb-vpn-1.9.1.AppImage
./dist/katacomb-vpn-1.9.1.AppImage
```
...
```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./katacomb-vpn-1.9.1.AppImage
```
