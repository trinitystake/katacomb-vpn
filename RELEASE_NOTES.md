...
- Front-load every password prompt in the ship flow
- Finish the disabled-cursor sweep on the provider modals
- Rebuild the Provider tab as a dashboard with a cached read-only mode
- Harden the provider console's chain paths
- Fixed typo
...
```bash
sudo apt install ./katacomb-vpn_1.5.0_amd64.deb
```
...
```bash
chmod +x katacomb-vpn-1.5.0.AppImage
./katacomb-vpn-1.5.0.AppImage
```
