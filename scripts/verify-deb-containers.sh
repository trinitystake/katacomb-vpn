...
if apt-get install -y -qq --no-install-recommends /tmp/app.deb file socat >/tmp/install.log 2>&1; then ok "deb installs with --no-install-recommends"; else no "deb install FAILED"; tail -20 /tmp/install.log; exit 1; fi
...
for img in "${IMAGES[@]}"; do
  printf '\n\033[1m== %s ==\033[0m\n' "$img"
  if docker run --rm -e IMAGE="$img" -e VERSION="$VERSION" -v "$DEB:/tmp/app.deb:ro" "$img" bash -c "$INNER"; then
    :
  else
    overall=1
  fi
