...
// SHA-256 hashes of the bundled binaries (verified at download time). Shared by
// the user-space app (vpn-manager) and the root daemon, which both refuse to
// execute a binary whose hash doesn't match. Node builtins only — importable by
// the standalone daemon (no Electron deps).
